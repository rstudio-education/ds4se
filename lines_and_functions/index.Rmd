...
## Which Language Uses More Functions per Line?
```{r fig.align='center',message=FALSE}
data <- data %>%
  group_by(Language) %>%
  mutate(func_density = FunctionDefs/Lines)
mean(data$func_density[data$Language=="Python"])
mean(data$func_density[data$Language=="JavaScript"])
plt = ggplot(data,aes(func_density,fill=Language))+
  geom_histogram(show.legend = FALSE)+
  facet_wrap(~Language)+
  theme_bw()
plt
