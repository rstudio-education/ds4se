---
title: "Learning to Code at Coding Bootcamps"
author: "Yim Register"
date: "8/21/2019"
...
Let's refer to one of our **factors** and take a look at a visualization of `Income`. I've selected `SchoolDegree` to see if the type of degree you have interacts with the salary in any way. What do you think?
```{r fig.width=10}
...
The above graph is a bit difficult to interpret. Instead, let's look at a more isolated comparison. It appears there might be no difference between people who had a Master's degree and those who had a Bachelor's degree (somewhat unexpected). Let's isolate those two groups using `dplyr` and the `filter()` command in order to get a clearer visualization of those groups.

```{r fig.width=10}
ba_and_ma <- data %>%
                  filter(SchoolDegree %in% c("bachelor's degree", "master's degree (non-professional)"))
...
### Inferential
Using our example from above, Bachelor's vs. trade schoolers and their `Income` values, let's look to see if we can find evidence that they are truly different phenomena.
