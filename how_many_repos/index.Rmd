...
plt = ggplot(merged,aes(n,num_projects_sum))+
  geom_point(size=.5)+
  geom_line(aes(n,justright),color="orange")+
...
third_polynomial <- lm(num_projects_sum~poly(n,3,raw=TRUE),data=train)
...
ggplot(future,aes(n,prediction))+
  ...
  geom_point(aes(n,num_projects_sum),color="black",size=1)+
  theme_bw()
